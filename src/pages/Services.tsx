...
export default function Services() {
    ...
    const fetchServices = async () => {
      try {
        ...
        if (res.data?.data?.length) {
          const merged = res.data.data.map((s: any) => ({
            ...s,
            rating: s.rating || 4.8,
            reviews: s.reviews || Math.floor(Math.random() * 150 + 50),
          }));
          setServices(merged);
          setFiltered(merged);
        }
  ...
  useEffect(() => {
    ...
    if (search) {
      result = result.filter(s => s.name.toLowerCase().includes(search.toLowerCase()) || s.description.toLowerCase().includes(search.toLowerCase()));
    }
    setFiltered(result);
  ...
  return (
    <div className="min-h-screen bg-[#060612] pt-16">
      ...
      <section className="relative py-20 overflow-hidden">
        <div className="glow-orb w-80 h-80 bg-purple-600 top-0 left-1/3 opacity-10" />
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="section-tag mx-auto w-fit mb-4">Our Services</div>
          <h1 className="text-5xl font-black text-white mb-4">Professional Career <span className="gradient-text">Services</span></h1>
          <p className="text-slate-400 max-w-xl mx-auto mb-10">From resume to job offer — we provide everything to accelerate your career journey.</p>
          <div className="max-w-md mx-auto relative">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-500" />
            <Input placeholder="Search services..." className="pl-12 h-12 text-base" value={search} onChange={e => setSearch(e.target.value)} />
          </div>
        </div>
      </section>
          ...
          <div className="flex flex-wrap gap-2 justify-center">
            {categories.map(cat => (
              <button key={cat} onClick={() => setActiveCategory(cat)}
                className={`px-5 py-2 rounded-full text-sm font-medium transition-all duration-200 ${activeCategory === cat ? 'bg-gradient-to-r from-purple-600 to-violet-600 text-white shadow-lg shadow-purple-500/25' : 'glass text-slate-400 hover:text-white hover:border-purple-500/40'}`}>
                {cat}
              </button>
            ))}
          </div>
      ...
      <section className="py-8 pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {filtered.length === 0 ? (
            <div className="text-center py-16 text-slate-400">No services found. Try a different search.</div>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filtered.map((service) => {
                const Icon = iconMap[service.category] || FileText;
                return (
                  <div key={service.id} className="glass rounded-2xl p-6 hover:-translate-y-2 transition-all duration-300 group flex flex-col">
                    <div className="flex items-start justify-between mb-4">
                      <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-purple-500 to-pink-600 flex items-center justify-center shadow-lg shadow-purple-500/30 group-hover:scale-110 transition-transform duration-300">
                    ...
                    <div className="flex items-center gap-3 mb-5 text-xs text-slate-500">
                      <span className="flex items-center gap-1"><Star className="w-3 h-3 fill-amber-400 text-amber-400" /> {service.rating}</span>
                      <span>({service.reviews} reviews)</span>
                      <span className="flex items-center gap-1 ml-auto"><Clock className="w-3 h-3" /> {service.duration}</span>
                    </div>
